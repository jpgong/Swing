...
public class UpdateFrame extends JFrame {
	...
	public UpdateFrame(MainFrame mainFrame, Student mainstudent) {
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {    //修改学生信息事件响应
				...
				if ((!textFieName.getText().equals(""))
						...
						&& (!textFieldClass.getText().equals(""))) {   //不为空，执行修改指令命令
					//修改学生信息
					stuDao.updateStudent(student, Integer.parseInt(textFieldId.getText()));     
		...
		btnUpdate.setFont(new Font("楷体", Font.BOLD, 17));
		panel.add(btnUpdate);
		...
		//修改窗口显示学生信息
		textFieldId.setText("" + mainstudent.getId());
		textFieName.setText(mainstudent.getName());
		if (mainstudent.getSex().equals("男")) {
			comboBoxSex.setSelectedItem("男");
		}else if (mainstudent.getSex().equals("女")) {
			comboBoxSex.setSelectedItem("女");
		}else {
			comboBoxSex.setSelectedItem("未知");
		}
		textFieldAge.setText("" + mainstudent.getAge());
		String[] hobby = mainstudent.getHobby().split("、");
		if (hobby.length > 0) {
			for (int i = 0; i < hobby.length; i++) {
				if (hobby[i].equals("旅游")) {
					checkBox1.setSelected(true);
				}
				if (hobby[i].equals("看书")) {
					checkBox2.setSelected(true);
				}
				if (hobby[i].equals("音乐")) {
					checkBox3.setSelected(true);
				}
				if (hobby[i].equals("上网")) {
					checkBox4.setSelected(true);
				}
				if (hobby[i].equals("羽毛球")) {
					checkBox5.setSelected(true);
				}
				if (hobby[i].equals("乒乓球")) {
					checkBox6.setSelected(true);
				}
			}
		}
		textFieldClass.setText(mainstudent.getSclass());
	}
}
